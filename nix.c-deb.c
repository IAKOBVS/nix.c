...
int nix_rev(char *restrict dest, const char *restrict src,
	    const size_t src_len);
int nix_rev_auto(char **restrict dest, const char *restrict src,
		 const size_t src_len);
int nix_rev_self_heap(char *restrict dest, const size_t src_len);
int nix_rev_self(char *restrict dest, const size_t src_len);
int nix_cut_delim(char *restrict dest, const char *restrict src, int n_str,
		  const int delim);
int nix_cut_first(char *restrict dest, const char *restrict src);
int nix_cut_first_delim(char *restrict dest, const char *restrict src,
			const int delim);
int nix_cut_last(char *restrict dest, const char *restrict src,
		 const size_t src_len);
int nix_cut_last_delim(char *restrict dest, const char *restrict src,
		       const size_t src_len, const int delim);
int nix_tee(char *restrict dest, const char *restrict flag,
	    const char *restrict filename);
int nix_head(char *restrict dest, const char *restrict filename);
int nix_cat(char *restrict dest, const char *restrict filename,
	    const size_t file_size);
int nix_cat_fast(char *restrict dest, const char *restrict filename,
		 const size_t file_size);
...
__attribute__((always_inline)) inline int nix_upper(char *restrict dest) {
  while ((*dest = (((*__ctype_b_loc())[(int)((*dest))] &
...
__attribute__((always_inline)) inline int nix_lower(char *restrict dest) {
  while ((*dest = (((*__ctype_b_loc())[(int)((*dest))] &
		    (unsigned short int)_ISalpha))
		      ? tolower(*dest)
		      : *dest))
...
inline int nix_rev(char *restrict dest, const char *restrict src,
		   const size_t src_len) {
  for (const char *restrict end = src + src_len - 1;
       (*dest++ = end >= src ? *end-- : '\0');)
    ;
  return 1;
}
...
__attribute__((always_inline)) inline size_t nix_size_of_file(
    const char *restrict filename) {
  struct stat st;
...
int nix_tee(char *restrict dest, const char *restrict flag,
  ...
  if (__builtin_expect(!!(!fp), 0)) goto ERROR;
  fputs(dest, fp);
  fclose(fp);
  return 1;
ERROR:
...
int nix_find(char *restrict dest, const char *restrict dir) {
  ...
  if (__builtin_expect(!!(!dp), 0)) goto ERROR;
...
int nix_find_auto(char **restrict dest, const char *restrict dir) {
  ...
  DIR *restrict dp = opendir(dir);
  if (__builtin_expect(!!(!dp), 0)) goto ERROR;
  if (__builtin_expect(!!(!(*dest = malloc(16000))), 0)) goto ERROR;
  size_t i = 0;
  for (size_t tmp_len, malloc_size = 16000; (ep = readdir(dp));) {
	...
	malloc_size *= 2;
      ...
      if (__builtin_expect(!!(!((*dest = realloc(*dest, malloc_size)))), 0))
	goto ERROR_FREE;
    }
  ...
  closedir(dp);
  if (__builtin_expect(!!(!(*dest = realloc(*dest, i + 1))), 0))
    goto ERROR_FREE;
  (*dest)[--i] = '\0';
  return i;
ERROR_FREE:
...
int nix_head(char *restrict dest, const char *restrict filename) {
  ...
  if (__builtin_expect(!!(!fp), 0)) return 0;
  fgets(dest, 256, fp);
  fclose(fp);
  return 1;
}
int nix_cat(char *restrict dest, const char *restrict filename,
  ...
  FILE *restrict fp = fopen(filename, "r");
  if (__builtin_expect(!!(!fp), 0)) goto ERROR;
  if (__builtin_expect(!!(!fread(dest, 1, file_size, fp)), 0)) goto ERROR_CLOSE;
  fclose(fp);
  dest[file_size] = '\0';
  return 1;
ERROR_CLOSE:
...
int nix_cat_fast(char *restrict dest, const char *restrict filename,
  ...
  FILE *restrict fp = fopen(filename, "r");
  if (__builtin_expect(!!(!fp), 0)) goto ERROR;
  if (__builtin_expect(!!(!fread_unlocked(dest, 1, file_size, fp)), 0))
    goto ERROR_CLOSE;
  fclose(fp);
  dest[file_size] = '\0';
  return 1;
...
int nix_cat_auto(char **restrict dest, const char *restrict filename) {
  ...
  if (__builtin_expect(!!(!(file_size)), 0)) goto ERROR;
  FILE *restrict fp = fopen(filename, "r");
  if (__builtin_expect(!!(!fp), 0)) goto ERROR;
  if (__builtin_expect(!!(!(*dest = malloc(file_size))), 0)) goto ERROR_CLOSE;
  if (__builtin_expect(!!(fread(*dest, 1, file_size, fp)), 0))
...
__attribute__((always_inline)) inline int nix_cut_first_delim(
    char *restrict dest, const char *restrict src, const int delim) {
  while ((*dest++ = *src++) != delim)
    ;
  *--dest = '\0';
  return 1;
}
inline int nix_cut_last(char *restrict dest, const char *restrict src,
			const size_t src_len) {
  src += src_len - 1;
  for (;; --src) {
    switch (*src) {
...
__attribute__((always_inline)) inline int nix_cut_last_delim(
    char *restrict dest, const char *restrict src, const size_t src_len,
    const int delim) {
  src += src_len - 1;
  while (*src-- != delim)
    ;
...
__attribute__((always_inline)) inline int nix_count(const int c) {
  int count = 0;
  while ((count += (const char *restrict src) ? 1 : 0), *src++)
    ;
  return count;
}
__attribute__((always_inline)) inline int nix_count_func(
    const char *restrict src, const int c) {
  int count = 0;
  while ((count += (*src == c) ? 1 : 0), *src++)
    ;
  return count;
}
__attribute__((always_inline)) inline int nix_count_digit(
    const char *restrict src) {
  int count = 0;
  while (
      (count +=
       ((*__ctype_b_loc())[(int)((*src))] & (unsigned short int)_ISdigit) ? 1
									  : 0),
      *src++)
    ;
  return count;
}
__attribute__((always_inline)) inline int nix_count_alpha(
    const char *restrict src) {
  int count = 0;
  while (
      (count +=
       ((*__ctype_b_loc())[(int)((*src))] & (unsigned short int)_ISalpha) ? 1
									  : 0),
      *src++)
    ;
  return count;
...
inline int nix_wc_char(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case ' ':
...
inline int nix_wc_char_comma(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case ',':
...
inline int nix_wc_char_dot(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '.':
...
inline int nix_wc_char_pipe(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '|':
...
inline int nix_wc_char_quote(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\'':
...
inline int nix_wc_char_double_quote(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '"':
...
inline int nix_wc_char_tab(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\t':
...
inline int nix_wc_char_nl(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_char_alpha(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_char_alpha_comma(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_char_alpha_dot(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_char_alpha_pipe(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_char_alpha_quote(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_char_alpha_double_quote(const char *restrict src) {
  int count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_comma(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case ',':
...
inline int nix_wc_word_pipe(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '|':
...
inline int nix_wc_word_dot(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '.':
...
inline int nix_wc_word_quote(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\'':
...
inline int nix_wc_word_double_quote(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '"':
...
inline int nix_wc_word_tab(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\t':
...
inline int nix_wc_word_nl(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_alpha(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_alpha_comma(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_alpha_pipe(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_alpha_dot(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_alpha_quote(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_alpha_double_quote(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\n':
...
inline int nix_wc_word_til_nl_pipe(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '|':
...
inline int nix_wc_word_til_nl_comma(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case ',':
...
inline int nix_wc_word_til_nl_dot(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '.':
...
inline int nix_wc_word_til_nl_quote(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '\'':
...
inline int nix_wc_word_til_nl_double_quote(const char *restrict src) {
  int in_word = 0, count = 0;
  for (;; ++src) {
    switch (*src) {
      case '"':
